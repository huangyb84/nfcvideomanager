...
app.get('/v/:id', async (req, res) => {
  try {
    const { id } = req.params;
    console.log(`Looking up NFC link: ${id}`);
    ...
    console.log('Query result:', { 
      nfcLink: nfcLink ? { id: nfcLink.id, short_id: nfcLink.short_id, has_video_contents: !!nfcLink.video_contents } : null,
      error: linkError,
      video_contents_count: nfcLink?.video_contents?.length || 0
    });

    // 如果新表没有，尝试从旧表查询（兼容模式）
    if (linkError || !nfcLink) {
      console.log('NFC link not found in nfc_links table, trying nfc_videos...');
      ...
      if (oldError || !oldData) {
        console.log('Not found in nfc_videos either');
    ...
    console.log('Current video found:', currentVideo ? { video_url: currentVideo.video_url, title: currentVideo.title } : null);
    
    if (!currentVideo) {
      console.log(`NFC Link ${id} has no current video. video_contents:`, JSON.stringify(nfcLink.video_contents, null, 2));
      return res.status(404).send(`
        <div style="text-align:center;padding:50px;font-family:system-ui;">
          <h1>⚠️ 未绑定视频</h1>
